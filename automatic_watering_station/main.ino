...
void loop(){
	...
	if (t > 26){ digitalWrite(ledPin,HIGH);}
	if (t > 26) { digitalWrite(vint,HIGH);}
	if (t > 26) digitalWrite(ledPin2,LOW); // дистанция в см
	float dist_cm = ultrasonic.Ranging(CM);
	Serial.println(dist_cm);// выводим дистанцию в порт
	if (dist_cm < 55) digitalWrite(ledPin4,HIGH);
	if (dist_cm > 55) digitalWrite(ledPin4,LOW); 
	if (dist_cm > 55) lcd.noBacklight(); 
	if (dist_cm < 55) lcd.backlight(); 
	if (dist_cm < 55 and t>26) digitalWrite(vint,HIGH);
	if (dist_cm > 55)
		return;
	if (t < 26)
		return;
